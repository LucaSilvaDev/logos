...
import { BOOK_ID_NAMES, BOOKS } from "@/lib/reading-plan"
...
export default function BibliaPage() {
  ...
  return (
    <>
      ...
      {/* Bottom verse action bar — 2 rows on mobile, single row on desktop */}
      {selectedVerses.size > 0 && (() => {
        ...
        return (
          ...
          >
            {/* Row 1 — reference + colors + close */}
            <div className="flex items-center gap-2 px-4 pt-3 pb-2">
              <span className="text-[#c9a654] text-[12px] font-serif font-medium shrink-0">
                {selectionRef()}
              </span>
              ...
              <div className="flex items-center gap-2.5 flex-1">
                {HL_COLORS.map(c => (
                  <button key={c.id} onClick={() => applyHighlightColor(c.id)} title={c.id}
                    className={cn(
                      "w-7 h-7 rounded-full shrink-0 transition-all duration-150 active:scale-90",
                      allSameColor(c.id) && "ring-2 ring-white/60 scale-110"
                    )}
            ...
            <div className="flex items-center px-2 pt-1">
              <button
                ...
                className={actionBtn} title="Marcador">
              ...
              <button onClick={copyVerse} className={actionBtn} title="Copiar">
                {copied
                  ? <Check className="w-5 h-5 text-[#5a9e72]" />
              ...
              <button onClick={() => { openNote(sortedNums[0]); setSelectedVerses(new Set()) }}
                className={actionBtn} title="Nota">
              ...
              {typeof navigator !== "undefined" && "share" in navigator && (
                <button onClick={shareVerse} className={actionBtn} title="Compartilhar">
                  <Share2 className="w-5 h-5" />
                  <span>Compartilhar</span>
                </button>
              )}

              <button onClick={downloadVerseImage} className={actionBtn} title="Imagem">
                <Download className="w-5 h-5" />
                <span>Imagem</span>
              </button>

              <button onClick={() => { fetchCompare(sortedNums[0]); setSelectedVerses(new Set()) }}
                className={actionBtn} title="Comparar">
